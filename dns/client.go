...
func (c *Client) storeCache(transport adapter.DNSTransport, question dns.Question, message *dns.Msg, timeToLive int) {
	if timeToLive == 0 {
		return
	}
	if c.disableExpire {
...
func (c *Client) lookupToExchange(ctx context.Context, transport adapter.DNSTransport, name string, qType uint16, options adapter.DNSQueryOptions, responseChecker func(responseAddrs []netip.Addr) bool) ([]netip.Addr, error) {
	...
	var exchangeResponseChecker func(response *dns.Msg) bool
	if responseChecker != nil {
		exchangeResponseChecker = func(response *dns.Msg) bool {
	...
	response, err := c.Exchange(ctx, transport, &message, options, exchangeResponseChecker)
	if err != nil {
		return nil, err
	}
